...
import { ShowImageDialog } from '@/features/nfts/components/StatusDialog'
import { useGetNFTDetail } from '@/features/nfts/services'
import { TokenDetailCard } from '@/features/tokens/components/TokenDetail'

export default function NFTDetail({ params }: { params: { address: string } }) {
	const mintAddress = params.address
	const { getExplorerUrl } = useCluster()
	const router = useRouter()

	const getNFTDetailData = useGetNFTDetail({ mintAddress })
	const NFTDetailData = getNFTDetailData.data?.data
	...
	const NFTAboutData = [
		{
			label: 'Name',
			value: NFTDetailData?.metadata?.name ?? '-'
		},
		{
			label: 'Symbol',
			value: NFTDetailData?.metadata?.symbol ?? '-'
		},
		{
			label: 'Collection',
			value: NFTDetailData?.metadata.collectionProperties?.name ?? '-'
		},
		{
			label: 'Family',
			value: NFTDetailData?.metadata?.metadataOffChain?.data.collection.family ?? '-'
		},
		{
			label: 'Mint Date',
			value: moment.unix(NFTDetailData?.createdAt ?? 0).format('D MMM, YYYY')
		}
	]
	...
	const metaDataInfo = [
		{
			...
			component: (
				<p className="truncate text-main-black text-base">
					{NFTDetailData?.metadata?.metadataOffChain.data.description ?? '-'}
				</p>
			)
		},
		{
			...
			component: (
				<div className="flex flex-wrap gap-2">
					{NFTDetailData?.metadata?.metadataOffChain.data.attributes?.map((attribute) => (
						<div
							key={attribute.traitType}
							className="bg-box w-auto text-sm px-3 h-8 py-1.5 rounded-[8px] flex items-center space-x-1.5"
						>
							<h4 className="font-semibold  text-sm text-main-black">{attribute.traitType}</h4>
		...
		},
		{
			...
			component: (
				<p className="truncate text-main-black text-base">
					{moment.unix(NFTDetailData?.createdAt ?? 0).format('D MMM, YYYY')}
				</p>
			)
		},
		{
		...
		},
		{
			...
			component: (
				<p className="truncate text-main-black text-base">
					{(NFTDetailData?.metadata.sellerFeeBasisPoints ?? 0) / 100}%
				</p>
			)
		},
		{
			...
			component: (
				<div className="flex items-center">
					<a
						...
						href={NFTDetailData?.metadata.metadataOffChain.link ?? ''}
						target="_blank"
						rel="noopener noreferrer"
					>
						{NFTDetailData?.metadata.metadataOffChain.link ?? '-'}
					</a>
					<CopyButton secretValue={NFTDetailData?.metadata.metadataOffChain.link ?? ''} />
				</div>
			)
		}
	]
	...
	return (
		<div className="2xl:px-[90px] xl:px-[80px] md:px-16 px-[15px]">
			<ShowImageDialog
				isOpen={isImageOpen}
				onOpenChange={setIsImageOpen}
				title={NFTDetailData?.metadata?.name ?? '-'}
				image={NFTDetailData?.metadata?.metadataOffChain.data.image ?? ''}
				description={''}
			/>
