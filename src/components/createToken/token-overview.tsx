...
export default function CreateTokenOverview(props: CreateTokenOverviewProps) {
	const tokenOverviewData: OverviewItemProps[] = [
		{
			label: 'Token Name',
			content: props.token_name ?? ''
		},
		{
			label: 'Symbol',
			content: props.token_symbol ?? ''
		},
		{
			label: 'Total Supply',
			content: Number(props.token_supply).toLocaleString() ?? ''
		},
		{
			label: 'Decimals',
			content: props.custom_decimals ?? ''
		},
		{
			label: 'Network',
			content: 'BBA Chain'
		...
		},
		{
			...
			content: (
				<div className="flex items-center">
					...
					>
						{props.token_icon?.name}
					</a>
					<CopyTooltip secretValue={props.image_link} />
				</div>
			)
		},
		{
